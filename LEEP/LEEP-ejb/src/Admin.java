...
public class Admin extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            Enseignant enseignant = enseignantFacade.create("blabla", "x", 12, 1, "notpass");
            Enseignement enseignement = enseignementFacade.create("EP", 400, enseignant);
            Cours cours = coursFacade.create("Coq2", enseignement);
            try {
                coursFacade.addHoraire(cours, 2017, 12, 16, 12, 0, 20);
            } catch (HorairePrisException ex) {
                Logger.getLogger(Admin.class.getName()).log(Level.SEVERE, null, ex);
            }
            Formation formation = formationFacade.create("EIDDDD3");
            formationFacade.addEnseignement(formation, enseignement);
            Etudiant etudiant = etudiantFacade.create("ddqdqzdq", "dds", 13, 1, "ddd", formation);
